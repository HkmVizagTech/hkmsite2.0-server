...
const blogController = {
  list: async (req, res) => {
    try {
      ...
      const [blogs, total] = await Promise.all([
        blogModel.find(filter).sort({ publishedAt: -1, createdAt: -1 }).skip(skip).limit(lim).select("-content").lean(),
        blogModel.countDocuments(filter),
      ]);

      res.status(200).json({ blogs, page: pageNum, limit: lim, total, totalPages: Math.ceil(total / lim) });
    } catch (err) {
  ...
  landing: async (req, res) => {
    try {
      ...
      const recents = await blogModel.find(base).sort({ publishedAt: -1 }).limit(5).select("-content").lean();
      const devotional = await blogModel.find(base).sort({ publishedAt: -1 }).limit(5).select("-content").lean();
      const counts = await blogModel.aggregate([
        { $match: base },
        { $group: { _id: "$category", count: { $sum: 1 } } },
      ]);
      const countMap = Object.fromEntries(counts.map((c) => [c._id, c.count]));
      const categories = BLOG_CATEGORIES.map((name) => ({ name, slug: slugify(name), count: countMap[name] || 0 }));
      const populatedCategories = categories.filter((c) => c.count > 0).slice(0, 6);
      const byCategory = await Promise.all(
        populatedCategories.map(async (cat) => {
          const items = await blogModel.find({ ...base, category: cat.name }).sort({ publishedAt: -1 }).limit(6).select("-content").lean();
          return { ...cat, items };
        })
      );
      let popular = await blogModel.find({ ...base, featured: true }).sort({ publishedAt: -1 }).limit(6).select("-content").lean();
      if (popular.length < 6) {
        const fill = await blogModel.find(base).sort({ views: -1, publishedAt: -1 }).limit(6 - popular.length).select("-content").lean();
        const have = new Set(popular.map((p) => String(p._id)));
        popular = [...popular, ...fill.filter((f) => !have.has(String(f._id)))];
      }
      const recent = await blogModel.find(base).sort({ publishedAt: -1 }).limit(7).select("-content").lean();
      res.status(200).json({ recents, devotional, categories, byCategory, popular, recent });
    } catch (err) {
  ...
  get: async (req, res) => {
    try {
      ...
      if (blog.status !== "published" && req.query.preview !== "1") return res.status(404).json({ message: "Blog not found" });
  ...
  related: async (req, res) => {
    try {
      ...
      const items = await blogModel.find({ _id: { $ne: id }, status: "published", category: current.category }).sort({ publishedAt: -1 }).limit(5).select("-content").lean();
  ...
  create: async (req, res) => {
    try {
      ...
      const uniqueSlug = await ensureUniqueSlug(slugify(slug || title));
  ...
  update: async (req, res) => {
    try {
      ...
      const patch = {
        ...
        author: buildAuthor({ ...req.body, authorAvatar }, req),
      ...
      if (req.body.featured !== undefined) patch.featured = req.body.featured === "true" || req.body.featured === true;
      if (req.body.slug !== undefined && req.body.slug !== existing.slug) patch.slug = await ensureUniqueSlug(slugify(req.body.slug), id);
